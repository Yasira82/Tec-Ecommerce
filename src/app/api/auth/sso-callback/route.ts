...
export async function GET(req: NextRequest) {
  ...
  // open apps through here with a one-time token (tec-app #258); a token used
  // twice (a second tap, or back to a stale page) answered
  // {"error":"replay_detected"} — a dead end on a phone (2026-09-26). The worst
  // case is now the visit as it was before those links existed.
  // No cookie is set on this 3xx (LAW 2); the reason is logged, never shown.
  const carryOn = (reason: string) => {
    console.warn(JSON.stringify({ level: 'warn', msg: 'auth.sso_callback_unusable', reason }));
  ...
  if (!secret) {
    console.error(JSON.stringify({ level: 'error', msg: 'auth.sso_not_configured' }));
...
<html><head><meta charset="utf-8"><title>TEC Ecommerce — Signing in…</title>
...
<div style="text-align:center"><div style="font-size:28px;font-weight:900">🏬 TEC Ecommerce</div>
