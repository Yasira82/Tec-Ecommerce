...
const buyRedirect = (product: Product) => {
  window.location.href = `${HUB_URL}/hub?pay=1`
    ...
    + `&return_url=${encodeURIComponent(`${ECOMMERCE_URL}/product/${product.id}`)}`
    + `&source=ecommerce`;
};
...
export default function ProductDetailPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#020205', color: '#fff', fontFamily: '-apple-system, BlinkMacSystemFont, system-ui, sans-serif', paddingBottom: 100 }}>
      ...
      {/* Image Gallery */}
      <div style={{ background: '#0a0a10', position: 'relative' }}>
        {/* Main Image */}
      ...
      <div style={{ padding: '20px 16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
          <span style={{ fontSize: 11, fontWeight: 700, color: '#4a4a5a', background: '#ffffff06', padding: '3px 10px', borderRadius: 20, textTransform: 'uppercase', letterSpacing: 1 }}>
            {product.category}
          </span>
          {product.rating > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              <span style={{ fontSize: 11, color: '#3a3a4a' }}>({product.reviews_count} reviews)</span>
            </div>
          )}
        </div>

        {/* Title */}
        <h1 style={{ fontSize: 22, fontWeight: 900, color: '#fff', margin: '0 0 8px', lineHeight: 1.2 }}>
          {product.title}
        </h1>

        {/* Merchant */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
          <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'linear-gradient(135deg,#d4af37,#b8882a)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 900, color: '#0a0800' }}>
            {product.merchant_name?.[0]?.toUpperCase()}
          </div>
          <button onClick={() => router.push(`/store/${product.merchant_id}`)}
            style={{ fontSize: 13, color: TEC_COLORS.gold, background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>
            @{product.merchant_name}
          </button>
        </div>

        {/* Description */}
        <div style={{ fontSize: 14, color: '#8b8b9a', lineHeight: 1.6, marginBottom: 20 }}>
          {product.description}
        </div>
        ...
        {product.stock === 0 && (
          <div style={{ fontSize: 12, color: '#ef4444', background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 8, padding: '6px 12px', marginBottom: 16, display: 'inline-block' }}>
            ❌ Out of stock
          </div>
        )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 20 }}>
          {[
            { label: 'Price',    value: formatPi(product.price)       },
            { label: 'Stock',    value: `${product.stock} units`       },
            { label: 'Reviews',  value: `${product.reviews_count}`     },
          ].map(s => (
