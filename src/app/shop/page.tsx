...
const buyRedirect = (params: { amount: number; memo: string; productId: string; returnUrl: string; source: string }) => {
...
const getPaymentReturnParams = () => {
  ...
  return { status: p.get('payment_status'), txid: p.get('txid'), paymentId: p.get('payment_id'), productId: p.get('product_id') };
};
  ...
  return (
    <div style={{ background: '#0d0d14', border: '1px solid #ffffff08', borderRadius: 16, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      <div style={{ width: '100%', aspectRatio: '1', background: '#ffffff05', position: 'relative', overflow: 'hidden' }}>
        {image && !imgError ? (
          <img src={image} alt={product.title} onError={() => setImgError(true)}
      ...
      <div style={{ padding: '12px 14px', flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: '#fff', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{product.title}</div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 'auto', paddingTop: 8 }}>
          <div style={{ fontSize: 16, fontWeight: 900, color: TEC_COLORS.gold }}>{product.price}π</div>
    ...
    <div style={{ minHeight: '100vh', background: '#020205', color: '#fff', fontFamily: '-apple-system, BlinkMacSystemFont, system-ui, sans-serif', paddingBottom: 32 }}>
      ...
      <div style={{ padding: '16px 16px 0' }}>
        <div style={{ position: 'relative', marginBottom: 12 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search products..." style={inputStyle} />
        </div>
