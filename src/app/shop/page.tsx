...
export default function ShopPage() {
  ...
  // ── payment handler (ADR-007 guard preserved) ──
      ...
      <main style={{ maxWidth:800, margin:'0 auto', padding:'16px 16px 48px' }}>
        ...
        <div style={{ position:'relative', marginBottom:14 }}>
          ...
          {searchQuery && (
            <button
              ...
              style={{ position:'absolute', right:12, top:'50%', transform:'translateY(-50%)', background:'none', border:'none', color:'#4a4a5a', cursor:'pointer', fontSize:16, lineHeight:1 }}
            >×</button>
          )}
        </div>
        ...
        {categories.length > 1 && (
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:8, marginBottom:12, flexWrap:'wrap' }}>
            <div style={{ display:'flex', gap:6, overflowX:'auto', flexShrink:1 }}>
              {categories.map(c => (
                <button
                  ...
                  style={{
                    padding:'6px 14px', borderRadius:20, fontFamily:'system-ui', fontSize:12, fontWeight:600,
            ...
            <select
              ...
              style={{
                padding:'7px 10px', borderRadius:10, fontFamily:'system-ui', fontSize:12, fontWeight:600,
                background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)',
                color:'#6b6b7a', cursor:'pointer', flexShrink:0,
        ...
        <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:16, flexWrap:'wrap' }}>
          ...
          {PRICE_BUCKETS.map(b => {
            ...
            return (
              <button
                key={b.label}
                onClick={() => { setMaxPriceBucket(b.max); setCustomMax(''); }}
                style={{
                  padding:'4px 12px', borderRadius:20, fontFamily:'system-ui', fontSize:11, fontWeight:600,
          ...
          <input
            ...
            placeholder="Max π"
            value={customMax}
            onChange={e => { setCustomMax(e.target.value); setMaxPriceBucket(null); }}
            style={{
              width:68, padding:'4px 8px', borderRadius:10, fontFamily:'system-ui', fontSize:11,
              background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)',
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
          <span style={{ fontFamily:'system-ui', fontSize:12, color:'#4a4a5a' }}>
            ...
            {hasActiveFilters && products.length !== filteredProducts.length && (
              <span style={{ color:'#6b6b7a' }}> of {products.length}</span>
            )}
          </span>
          {hasActiveFilters && (
            <button
              onClick={clearAllFilters}
              style={{ fontFamily:'system-ui', fontSize:11, color:'#4a4a5a', background:'none', border:'1px solid rgba(255,255,255,0.08)', borderRadius:8, padding:'3px 10px', cursor:'pointer' }}
            >
              ✕ Clear filters
            </button>
          )}
        ...
        {filteredProducts.length === 0 ? (
          <div style={{ textAlign:'center', padding:'60px 0' }}>
            ...
            <button onClick={clearAllFilters} style={{ fontFamily:'system-ui', fontSize:12, color:'#d4af37', background:'none', border:'1px solid rgba(212,175,55,0.25)', borderRadius:10, padding:'8px 18px', cursor:'pointer' }}>
              Clear filters
            </button>
          </div>
        ) : (
