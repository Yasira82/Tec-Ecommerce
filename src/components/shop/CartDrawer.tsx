      ...
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, zIndex: 300, width: 320, maxWidth: '92vw', background: '#0B1020', borderLeft: '1px solid rgba(251,191,36,0.1)', transform: isOpen ? 'translateX(0)' : 'translateX(100%)', transition: 'transform 0.3s cubic-bezier(0.16,1,0.3,1)', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
        <div style={{ padding: '18px 20px 14px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            {items.length > 0 && <button onClick={onClear} style={{ fontFamily: 'system-ui', fontSize: 11, color: '#4a4a5a', background: 'none', border: 'none', cursor: 'pointer', padding: '4px 8px' }}>Clear all</button>}
            <button onClick={onClose} style={{ width: 28, height: 28, borderRadius: '50%', background: 'rgba(255,255,255,0.07)', border: 'none', color: '#6b6b7a', fontSize: 17, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
          </div>
        </div>
              ...
              <div key={product.id} style={{ display: 'flex', gap: 12, padding: '12px 20px', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <button onClick={() => onUpdateQty(product.id, qty - 1)} style={{ width: 26, height: 26, borderRadius: 8, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', color: '#aaa', fontSize: 15, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1 }}>−</button>
                    <span style={{ fontFamily: 'system-ui', fontSize: 13, fontWeight: 700, color: '#fff', minWidth: 20, textAlign: 'center' }}>{qty}</span>
                    <button onClick={() => onUpdateQty(product.id, qty + 1)} style={{ width: 26, height: 26, borderRadius: 8, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', color: '#aaa', fontSize: 15, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1 }}>+</button>
                    <button onClick={() => onRemove(product.id)} style={{ marginLeft: 4, background: 'none', border: 'none', color: '#4a4a5a', cursor: 'pointer', fontSize: 14, padding: '2px 4px', transition: 'color 0.15s' }} onMouseEnter={e => (e.currentTarget.style.color = '#ef4444')} onMouseLeave={e => (e.currentTarget.style.color = '#4a4a5a')}>🗑</button>
                  </div>
                </div>
              </div>
            );
          ...
          <div style={{ padding: '16px 20px', borderTop: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
            ...
            {(status === 'error' || status === 'cancelled') && (
              <div style={{ marginBottom: 10, textAlign: 'center' }}>
                ...
                <button onClick={resetStatus} style={{ fontFamily: 'system-ui', fontSize: 11, color: '#6b6b7a', background: 'none', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '4px 12px', cursor: 'pointer' }}>Try again</button>
              </div>
            )}
            <button onClick={handleCheckout}
              ...
              {status === 'creating' ? '⏳ Preparing…' : status === 'paying' ? '🔷 Pay in Pi Wallet…' : status === 'success' ? '✅ Done!' : '🔷 Checkout with Pi'}
            </button>
            <p style={{ fontFamily: 'system-ui', fontSize: 10, color: '#3a3a4a', textAlign: 'center', marginTop: 10, lineHeight: 1.5 }}>Secure payment via Pi Network</p>
          </div>
        )}
      </div>
    </>
