...
const ROUTE_SRC = readFileSync(join(process.cwd(), 'src/app/api/auth/sso-callback/route.ts'), 'utf8');
const DEFAULT = ROUTE_SRC.match(/DEFAULT_REDIRECT = '([^']+)'/)?.[1] as string;
...
describe('sso-callback with a token it cannot use', () => {
  ...
  it('never leaves the origin', async () => {
    for (const bad of ['//evil.example', '/\\evil.example', 'https://evil.example']) {
      expect(wentOnTo(await land(null, bad))).toBe(DEFAULT);
    }
  });
});
